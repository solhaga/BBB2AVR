...
#my $MatchRegExString = "^(ir|rfm) [0-9]{1,2} ([0-1]{5}|[0-9A-F]{1,}) ([A-E]|[0-9A-F]{1,}) [0-1]"; #<-- Have to be modified for each application/device
my $MatchRegExString = "^(temp) [0-9]{1,2}"; #<-- Have to be modified for each application/device
...
ECMDDevice_Parse($$) {
  ...
  foreach my $d (keys %defs) {
  	if($defs{$d}{TYPE} eq "ECMDDevice" && $defs{$d}{IODev} eq $IOhash) { ## Find ECMDDevice entrys
  		## Found valid ECMDDevice hash
  		my $classname = $defs{$d}{fhem}{classname};
  		my $ECMDDeviceParams = $defs{$d}{fhem}{params}; #Device HouseCode Protocol
  		my $recvs = $IOhash->{fhem}{classDefs}{$classname}{recvs};
  		my $s;
  		
  		foreach $s (keys % {$recvs}) { #z.B. on und off
  			my $DeviceCmd = $recvs->{$s}{cmd};
  			$DeviceCmd =~ s/(^{"|"}$)//g;
  			$DeviceCmd =~ s/(^\s+|\s+$)//g;
  			foreach my $p (keys % {$ECMDDeviceParams}) { ## replace parameter strings "%HouseCode" with Device parameter values
  				$DeviceCmd =~ s/%$p/$ECMDDeviceParams->{$p}/g;
  			}
  			if( $DeviceCmd eq $rmsg ) { ## match Device found -> exit loop ans safe state+devicename
  				$s =~ s/(^\s+|\s+$)//g;
  				readingsSingleUpdate($defs{$d}, "state", $s, 1);
					Log GetLogLevel($d, 4), "ECMDDevice $d $s";
					push(@found, $d);
  				#return $d; ## es sollen alle gültigen Devices gefunden werden, nicht nur eins
  			}
  		}
  	}
  }
  ...
  return "UNDEFINED ECMDDevice message $rmsg";
}
...
ECMDDevice_Set($@)
{
        ...
        if(!defined($IOhash->{fhem}{classDefs}{$classname}{sets}{$cmdname})) {
					...
					#Log 5, "ECMDDevice TEST: $name $classname $type cmd: $cmdname";
