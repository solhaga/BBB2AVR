...
sub ECMD_Read($) {
  ...
  Log 5, "ECMD: PARTIAL/RAW: $data/$buf"; 
  $data .= $buf;
  ...
  while($data =~ m/\n/) {
    ...
    ($rmsg,$data) = split("\n", $data, 2);
    $rmsg =~ s/\r//;
    Dispatch($hash, $rmsg, undef); ##Dispatch Reading to ECMDDevice
  }
  $hash->{PARTIAL} = $data;
}
...
# Consumer
  ...
  $hash->{AttrList}= "classdefs nonl noWriteFeedback loglevel:0,1,2,3,4,5";
}
...
ECMD_DoInit($)
{
  ...
  #ECMD_SimpleWrite($hash, "ram");
  #my ($err,$ram)= ECMD_ReadAnswer($hash, "ram");
  #if(!($err)) {
	#  Log 2, "ECMD Ram used: $ram";
	#  $hash->{RAM} = $ram;
	#}
	
  #ECMD_SimpleWrite($hash, "V");
  #my ($err2,$version)= ECMD_ReadAnswer($hash, "V");
  #if(!($err)) {
...
########################
# Liest daten aus der RS232 und gibt erst die daten zurück wenn ein \n vorhanden ist
sub
ECMD_SimpleRead($)
{
  my ($hash) = @_;
...
ECMD_Clear($)
{
  ...
  # Clear the pipe
  $hash->{RA_Timeout} = 2; ## Initialisierungszeit des µControlers
  for(;;) {
...
ECMD_EvalClassDef($$$)
{
	...
	my $cont= "";
		...
		$cont= "";
                ...
                if($cmd eq "nonl") {
                ...
                } elsif($cmd eq "set" || $cmd eq "get" || $cmd eq "recv") {
                        ...
                        if($spec eq "params") {
                                if($cmd eq "set") {
                                        ...
                                        Log 5, "$name: recv $cmdname has parameters $arg";
                                        $hash->{fhem}{classDefs}{$classname}{recvs}{$cmdname}{params}= $arg;
                                }
                        } elsif($spec eq "cmd") {
                                if($arg !~ m/^{.*}$/s) {
                                ...
                                if($cmd eq "set") {
                                        ...
                                        Log 5, "$name: recv $cmdname command defined as $arg";
                                        $hash->{fhem}{classDefs}{$classname}{recvs}{$cmdname}{cmd}= $arg;
                                }
                                ...
                                if($cmd eq "set") {
                                        ...
                                        Log 5, "$name: recv $cmdname postprocessor defined as $arg";
                                        $hash->{fhem}{classDefs}{$classname}{recvs}{$cmdname}{postproc}= $arg;
                                }
                        }
                } else {
...
ECMD_Write($$)
{
  ...
  my $noWriteFeedback= AttrVal($hash->{NAME}, "noWriteFeedback", 0);
  foreach my $ecmd (@ecmds) {
        Log 5, "ECMD: $hash->{NAME} sending $ecmd";
        ECMD_SimpleWrite($hash, $ecmd, $nonl);
        if($noWriteFeedback == 0) {
	        ($err, $answer)=ECMD_ReadAnswer($hash, "$ecmd");
  	      if(defined($answer)) {
		        push @r, $answer;
		        Log 5, "ECMD: hash->{NAME} answer <$answer>";
					}
				}
  }
	return join("\n", @r);
