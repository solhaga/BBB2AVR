...
sub GPIO4_Define($$) {
	...
	if ($a[2] eq "BUSMASTER") {
		...
		if ($family eq "28" || $family eq "10") {
			...
			$hash->{fhem}{interfaces} = "T";   # <--  hier Änderung von "temperature" nach "T"
...
#                hier Original 3 NachKomman-Stellen
#                my $temp = $1/1000.0;
#
#                det / 18.Juli 2013   geändert auf 1 Nachkomma-Stelle   für DS1820 
#		my $temp = sprintf("%.1f",$1/1000.0 - 1.5);    #  1.5° zuviel !!!
		...
		readingsBulkUpdate($hash,"T",$temp);  #  <-- hier Änderung von "temperature" nach "T"
